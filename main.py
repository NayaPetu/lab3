...
# def read_employee_role(emp_id: int):
#     """Роль сотрудника"""
...
@app.get("/", tags=["Главная"])
def root():
    """Главная страница API системы документооборота"""
    return {
        "message": "API документооборота запущено!",
        "documentation": "/docs",
        "version": "2.0",
        "author": "Петурова Надежда Игоревна"
    }

# ====================== ДОКУМЕНТЫ - ОСНОВНЫЕ ОПЕРАЦИИ ======================

@app.get("/documents", response_model=List[document], tags=["Документы"])
def read_documents():
    """Получить все документы системы"""
    return get_all_documents()

@app.get("/documents/on-approval", response_model=List[document], tags=["Документы"])
def read_documents_on_approval():
    """Получить документы, находящиеся на согласовании"""
    return get_documents_on_approval()

@app.get("/documents/author/{author_id}", response_model=List[document], tags=["Документы"])
def read_documents_by_author(author_id: int):
    """Получить документы конкретного автора"""
    docs = get_documents_by_author(author_id)
    if not docs:
        raise HTTPException(status_code=404, detail="Документы не найдены")
    return docs

@app.post("/documents", response_model=document, status_code=status.HTTP_201_CREATED, tags=["Документы"])
def create_document(doc: document):
    """Создать новый документ"""
    with Session(engine) as session:
        # Автоматически ставим статус "в работе"
...
@app.put("/documents/{doc_id}", response_model=document, tags=["Документы"])
def update_document(doc_id: int, updated_data: document):
    """Обновить документ"""
    with Session(engine) as session:
        db_doc = session.get(document, doc_id)
...
@app.delete("/documents/{doc_id}", status_code=status.HTTP_204_NO_CONTENT, tags=["Документы"])
def delete_document(doc_id: int):
    """Удалить документ"""
    with Session(engine) as session:
        doc = session.get(document, doc_id)
...
# ====================== ИСТОРИЯ И СОГЛАСОВАНИЯ ======================

@app.get("/documents/{doc_id}/history", response_model=List[documentflow], tags=["История и согласования"])
def read_document_history(doc_id: int):
...
@app.get("/documents/{doc_id}/approvals", response_model=List[approval], tags=["История и согласования"])
def read_document_approvals(doc_id: int):
    """Получить согласования по документу"""
    approvals = get_approvals_for_document(doc_id)
    if not approvals:
        raise HTTPException(status_code=404, detail="Согласования не найдены")
    return approvals
...
@app.post("/documentflow", response_model=documentflow, status_code=status.HTTP_201_CREATED, tags=["Работа с документами"])
def send_document(flow: documentflow):
    """Отправить документ на согласование/исполнение"""
    with Session(engine) as session:
        flow.sent_at = datetime.utcnow()
...
@app.post("/approvals", response_model=approval, status_code=status.HTTP_201_CREATED, tags=["Работа с документами"])
def approve_document(approval_data: approval):
    """Согласовать документ"""
    with Session(engine) as session:
        approval_data.approval_date = datetime.utcnow()
...
@app.get("/employee/{emp_id}/role", tags=["Справочники"])
def read_employee_role(emp_id: int):
    """Получить роль сотрудника"""
    emp_role = get_employee_role(emp_id)
    if not emp_role:
        raise HTTPException(status_code=404, detail="Сотрудник или роль не найдены")
    return {"id": emp_role.id, "name": emp_role.name}

@app.get("/department/{dept_id}/head", tags=["Справочники"])
def read_department_head(dept_id: int):
...
@app.get("/roles", response_model=List[role], tags=["Справочники"])
def read_roles():
    """Получить все роли в системе"""
    with Session(engine) as session:
        return session.exec(select(role)).all()

@app.get("/departments", response_model=List[department], tags=["Справочники"])
def read_departments():
    """Получить все подразделения"""
    with Session(engine) as session:
        return session.exec(select(department)).all()

@app.get("/statuses", response_model=List[dstatus], tags=["Справочники"])
def read_statuses():
    """Получить все возможные статусы документа"""
    with Session(engine) as session:
        return session.exec(select(dstatus)).all()

@app.get("/employees", response_model=List[employee], tags=["Справочники"])
def read_employees():
    """Получить всех сотрудников"""
    with Session(engine) as session:
        return session.exec(select(employee)).all()

@app.get("/document-types", response_model=List[dtype], tags=["Справочники"])
